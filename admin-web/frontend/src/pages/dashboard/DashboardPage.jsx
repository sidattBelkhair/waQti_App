  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-full overflow-x-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 p-4 sm:p-6 min-w-0">
            ...
            {stats.topEtablissements.map((e, i) => (
              <div key={i} className="flex items-center justify-between gap-3 py-2">
                <div className="flex items-center gap-3 min-w-0">
                  <span className="w-8 h-8 shrink-0 bg-blue-100 text-blue-600 rounded-lg flex items-center justify-center text-sm font-bold">{i + 1}</span>
                  <span className="text-sm font-medium text-slate-700 truncate">{e.nom || 'Inconnu'}</span>
                  <span className="text-xs text-slate-400 shrink-0">{e.type}</span>
                </div>
                <span className="text-sm font-bold text-slate-800 shrink-0">{e.totalTickets} tickets</span>
              </div>
            ))}
          </div>
        </div>
